...
for i in range(1,5):
	for j in range (i,5):
		currxi = np.genfromtxt('/media/juancordero/Gaia/Documentos/Manchester/KIDS/CosmoParams/MyCorrelationFunctions/xi_' + str(i) + '-' + str(j))
...
def gaus1(x,a,sigma):
	return a*np.exp(-(x-0.2)**2/(2*sigma**2))
...
for i in range(4):
	...
	if i == 0:
		popt, pcov = curve_fit(gaus1,ccfile[0:9,0],ccfile[0:9,1])
		fit = gaus1(ccfile[:,0],*popt)
	else:
		popt, pcov = curve_fit(gaus,ccfile[:,0],ccfile[:,1])
		fit = gaus(ccfile[:,0],*popt)
...
thdulist.writeto('KiDS_like_CC_fit_myxi.fits', clobber = True)
...
plt.savefig('nz_CC_fit_myxi.png', dpi=200)
#plt.savefig('nz_CC_fit_myxi.pdf', dpi=200)
